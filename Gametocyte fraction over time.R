...
mod0 = bam(loggamfrac ~ s(weeks, bs="bs")+ s(infection_id, bs="re"), 
           family=gaussian(),
           data=dtrm)
summary(mod0)
gam.vcomp(mod0)
...
newd$loggamfrac = preds$fit
newd$loggamfrac.lci=preds$fit-1.96*preds$se.fit
newd$loggamfrac.uci=preds$fit+1.96*preds$se.fit
newd$gamfrac = exp(newd$loggamfrac)-0.001
newd$gamfrac.lci = exp(newd$loggamfrac.lci)-0.001
newd$gamfrac.uci = exp(newd$loggamfrac.uci)-0.001


plot1.gamfrac=ggplot(data=newd, aes(x=weeks,y=gamfrac))+
  geom_line(size=1.2)+
  geom_ribbon(aes(ymin=gamfrac.lci,ymax=gamfrac.uci),alpha=0.2)+
  theme_prism()+
...
#ggsave("gamfrac_dens.pdf",plot=plot1,device="pdf",dpi=300, width=6, height=4)

plot1.c=ggarrange(plot1+rremove("xlab"),
                  plot1.gam+rremove("xlab"),
                  plot1.gamfrac+rremove("xlab"), nrow=1,
                  labels=c("A","B","C"))
...
ggsave("figures/all_longitudinal_densities.pdf",plot=plot1.c,device="pdf",dpi=300, width=12, height=8)
...
mod.c = bam(loggamfrac ~ s(weeks, bs="bs")+ 
              s(logqpcr,bs="bs")+
...
gamfracbypar =  expand.grid(weeks=4,logqpcr=seq(-2,5,by=0.01),tend=1,infection_id="1") %>% 
  add_hazard(mod.c,exclude="s(infection_id)") 


plot.gfp=ggplot(data=gamfracbypar, aes(x=(exp(logqpcr)-0.1),y=hazard-0.001))+
  geom_line(size=1.2)+
  geom_ribbon(aes(ymin=ci_lower-0.001,ymax=ci_upper-0.001),alpha=0.2)+
  theme_prism()+
...
plot1.d=ggarrange(plot.gp+rremove("xlab"),
                  plot.gfp+rremove("xlab"), nrow=1,
                  labels=c("A","B"))

plot1.d=annotate_figure(plot1.d,
                        bottom = text_grob("Parasite density/\U00B5L",face="bold",size=14))

ggsave("figures/gam_dens_and_frac_by_parasitedensity.pdf",plot=plot1.d,device="pdf",dpi=300, width=12, height=8)
...
mod.c = bam(loggamfrac ~ age2+age3+s(weeks, bs="bs")+ 
              s(infection_id, bs="re"), 
            family=gaussian(),
            data=dtrm)
summary(mod.c)
...
mod.c = bam(loggamfrac ~ male+s(weeks, bs="bs")+ s(infection_id, bs="re"), 
            family=gaussian(),
            data=dtrm)
summary(mod.c)
...
mod.c = bam(loggamfrac ~ hbs.homo+hbs.hetero+s(weeks, bs="bs")+ s(infection_id, bs="re"), 
            family=gaussian(),
            data=dtrm)
summary(mod.c)
...
mod.c = bam(loggamfrac ~ g6pd376.homo+g6pd376.hetero+s(weeks, bs="bs")+ s(infection_id, bs="re"), 
            family=gaussian(),
            data=dtrm)
summary(mod.c)
...
mod.c = bam(loggamfrac ~ g6pd202.homo+g6pd202.hetero+
              s(weeks, bs="bs")+ 
              s(infection_id, bs="re"), 
            family=gaussian(),
            data=dtrm)
...
mod.c = bam(loggamfrac ~ age2+ age3+ male+ 
              s(weeks, bs="bs")+ 
              s(logqpcr,bs="bs")+
              s(infection_id, bs="re"), 
            family=gaussian(),
...
mod.c = bam(loggamfrac ~ age2+ age3+ male+ hbs.homo+hbs.hetero+
...
mod.c = bam(loggamfrac ~ age2+ age3+ male+ g6pd376.homo+g6pd376.hetero+
              s(weeks, bs="bs")+ s(logqpcr,bs="bs")+ s(infection_id, bs="re"), 
            family=gaussian(),
            data=dtrm)
summary(mod.c)
...
mod.c = bam(loggamfrac ~ age2+ age3+ male+ g6pd202.homo+g6pd202.hetero+
              s(weeks, bs="bs")+ 
...
mod.c = bam(loggamfrac ~ age2+ age3+ male+ moib+
              s(weeks, bs="bs")+ 
              s(logqpcr,bs="bs")+ s(infection_id, bs="re"), 
            family=gaussian(),
            data=dtrm)
...
mod.c = bam(loggamfrac ~ age2+ age3+ 
              s(weeks, bs="bs")+ 
              s(logqpcr,bs="bs")+
              s(infection_id, bs="re"), 
            family=gaussian(),
...
mod.c = bam(loggamfrac ~ g6pd376.homo+g6pd376.hetero+
              s(weeks, bs="bs")+ s(logqpcr,bs="bs")+ s(infection_id, bs="re"), 
            family=gaussian(),
            data=dtrm)
summary(mod.c)
...
xlsx::write.xlsx2(resdt,"results_estimated.xlsx",sheetName = "gametocyte fraction",row.names = FALSE,append=TRUE)
